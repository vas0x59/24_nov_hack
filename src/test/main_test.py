...
while not rospy.is_shutdown():
    ...
    left_points_pub.publish(
        PoseArray(poses=[Pose(position=Point(x=i[0], y=i[1], z=0)) for i in list(left_points) + list(forward_points)],
                  header=Header(frame_id="base_scan")))
    rospy.sleep(0.1)